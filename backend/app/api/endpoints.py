...
class ConcessionOrderContext(BaseModel):
    user_id: str = Field(..., min_length=3, max_length=50, description="Unique identifier for the user.")
    stand_id: str = Field(..., min_length=2, max_length=20, pattern=r"^[A-Za-z0-9-]+$")
    menu_item: str = Field(..., min_length=2, max_length=100)
    distance_meters: float = Field(..., ge=0, le=10000, description="Distance in meters from the stand.")

@router.post("/jit-concessions/order")
@limiter.limit("10/minute")
async def place_jit_order(request: Request, order: ConcessionOrderContext, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        if cache_key in distance_cache:
            ...
            gmaps_data = await google_services.get_distance_matrix(
                origins=f"user_{order.user_id}",
                destinations=order.stand_id
            )
        ...
        return {
            ...
            "details": {
                ...
                "gmaps_verified": True if gmaps_data.get("status") == "success" else False
            }
        }
